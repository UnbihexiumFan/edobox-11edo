...
export class Instrument {
    ...
    public static frequencyFromPitch(pitch: number): number {
        return 440.0 * Math.pow(2.0, (pitch - 69.0) / Config.edo);
    }
...
export class Song {
    ...
    public toBase64String(): string {
        ...
        for (let channelIndex: number = 0; channelIndex < this.getChannelCount(); channelIndex++) {
            ...
            const octaveOffset: number = (isNoiseChannel || isModChannel) ? 0 : channel.octave * Config.edo;
        ...
        if (jsonObject["key"] != undefined) {
            if (typeof (jsonObject["key"]) == "number") {
                ...
                if (index != undefined) {
                    ...
                    if (index < 0) index += Config.edo;
                    index = index % Config.edo;
                    this.key = index;
                }
            }
        }
...
export class Synth {
    ...
    private computeTone(song: Song, channelIndex: number, samplesPerTick: number, tone: Tone, released: boolean, shouldFadeOutFast: boolean): void {
        ...
        if (effectsIncludeDetune(instrument.effects) || this.isModActive(Config.modulators.dictionary["song detune"].index, channelIndex, tone.instrumentIndex)) {
            ...
            intervalStart += Synth.detuneToCents(modDetuneStart) * envelopeStart * Config.edo / (12.0 * 100.0);
            intervalEnd += Synth.detuneToCents(modDetuneEnd) * envelopeEnd * Config.edo / (12.0 * 100.0);
        }
        ...
        if (instrument.type == InstrumentType.fm) {
            ...
            const freqEndRatio: number = Math.pow(2.0, (intervalEnd - intervalStart) * intervalScale / Config.edo);
            ...
            if (tone.pitchCount > 1 && (chord.arpeggiates || chord.customInterval)) {
                ...
                if (chord.customInterval) {
                    ...
                    specialIntervalMult = Math.pow(2.0, intervalOffset / Config.edo);
            ...
            if (instrument.type == InstrumentType.chip || instrument.type == InstrumentType.customChipWave || instrument.type == InstrumentType.harmonics || instrument.type == InstrumentType.pickedString) {
                ...
                const unisonAStart: number = Math.pow(2.0, (unison.offset + unison.spread) * unisonEnvelopeStart / Config.edo);
                const unisonAEnd: number = Math.pow(2.0, (unison.offset + unison.spread) * unisonEnvelopeEnd / Config.edo);
                const unisonBStart: number = Math.pow(2.0, (unison.offset - unison.spread) * unisonEnvelopeStart / Config.edo) * specialIntervalMult;
                const unisonBEnd: number = Math.pow(2.0, (unison.offset - unison.spread) * unisonEnvelopeEnd / Config.edo) * specialIntervalMult;
			...
			if (instrument.type == InstrumentType.supersaw) {
				...
				for (let i = 0; i < Config.supersawVoiceCount; i++) {
					...
					tone.supersawUnisonDetunes[i] = Math.pow(2.0, curvedSpread * offset / Config.edo);
				}
