    ...
    for (const note of pattern.notes) {
        for (const pitch of note.pitches) {
            for (const pin of note.pins) {
                const key: number = (pitch + pin.interval) % Config.edo;
                if (!flags[key]) {
                    flags[key] = true;
                }
            }
...
export function generateScaleMap(oldScaleFlags: ReadonlyArray<boolean>, newScaleValue: number): number[] {
    ...
    sparsePitchMap.push([Config.edo, Config.edo]);
    newScale.push(Config.edo);
...
class ChangeTransposeNote extends UndoableChange {
    ...
    constructor(doc: SongDocument, channelIndex: number, note: Note, upward: boolean, ignoreScale: boolean = false, octave: boolean = false) {
        ...
        for (let i: number = 0; i < this._oldPitches.length; i++) {
            ...
            if (octave && !isNoise) {
                if (upward) {
                    pitch = Math.min(maxPitch, pitch + Config.edo);
                } else {
                    pitch = Math.max(0, pitch - Config.edo);
                }
            } else {
                if (upward) {
                    for (let j: number = pitch + 1; j <= maxPitch; j++) {
                        if (isNoise || ignoreScale || Config.scales[doc.song.scale].flags[j % Config.edo]) {
                            pitch = j;
                            break;
                        }
                    }
        ...
        for (const oldPin of this._oldPins) {
            ...
            if (octave && !isNoise) {
                if (upward) {
                    interval = Math.min(max, interval + Config.edo);
                } else {
                    interval = Math.max(min, interval - Config.edo);
                }
            } else {
                if (upward) {
                    for (let i: number = interval + 1; i <= max; i++) {
                        if (isNoise || ignoreScale || Config.scales[doc.song.scale].flags[i % Config.edo]) {
                            interval = i;
                            break;
                        }
                    }
    ...
    constructor(doc: SongDocument, pattern: Pattern, scaleMap: number[]) {
        ...
        for (const note of pattern.notes) {
            ...
            for (let i: number = 0; i < note.pitches.length; i++) {
                ...
                const transformedPitch: number = scaleMap[pitch % Config.edo] + (pitch - (pitch % Config.edo));
            ...
            for (const oldPin of note.pins) {
                ...
                if (interval < min) interval = min;
                if (interval > max) interval = max;
                const transformedInterval: number = scaleMap[interval % Config.edo] + (interval - (interval % Config.edo));
