...
export class RecordingSetupPrompt implements Prompt {
	...
	private _renderKeyboardLayoutPreview = (): void => {
		...
		for (let rowIndex: number = 0; rowIndex < 4; rowIndex++) {
			...
			const spacer: HTMLDivElement = div({style: "width: " + (rowIndex * Config.edo) + "px; height: 20px; flex-shrink: 0;"});
			...
			for (let colIndex: number = 0; colIndex < rowLengths[rowIndex]; colIndex++) {
				...
				if (pitch != null) {
					const scalePitch: number = pitch % Config.edo;
					if (scale[scalePitch]) {
						if (scalePitch == 0) {
							key.style.background = ColorConfig.tonic;
						} else if (scalePitch == Math.round(Config.edo*Math.log2(3/2)) && this._doc.prefs.showFifth) {
					...
					const pitchNameIndex: number = (scalePitch + Config.keys[this._doc.song.key].basePitch) % Config.edo;
					key.textContent = Piano.getPitchName(pitchNameIndex, scalePitch, Math.floor(pitch / Config.edo));
				}
			}
		}
	}
