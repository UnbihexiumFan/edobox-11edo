...
export class PatternEditor {
    ...
    constructor(private _doc: SongDocument, private _interactive: boolean, private _barOffset: number) {
        for (let i: number = 0; i < Config.edo; i++) {
    ...
    private _snapToPitch(guess: number, min: number, max: number): number {
        ...
        if (scale[Math.floor(guess) % Config.edo] || this._doc.song.getChannelIsNoise(this._doc.channel) || this._doc.song.getChannelIsMod(this._doc.channel)) {
    ...
    public render(): void {
        ...
        this._octaveOffset = (this._doc.channel >= this._doc.song.pitchChannelCount) ? 0 : this._doc.song.channels[this._doc.channel].octave * Config.edo;
        ...
        this._octaveOffset = (this._doc.channel >= this._doc.song.pitchChannelCount) ? 0 : this._doc.getBaseVisibleOctave(this._doc.channel) * Config.edo;
        ...
        if (this._renderedBeatWidth != beatWidth || this._renderedPitchHeight != this._pitchHeight) {
            ...
            this._svgNoteBackground.setAttribute("height", "" + (this._pitchHeight * Config.edo));
            ...
            for (let j: number = 0; j < Config.edo; j++) {
                const rectangle: SVGRectElement = this._backgroundPitchRows[j];
                const y: number = (Config.edo - j) % Config.edo;
        ...
        if (this._renderedFifths != this._doc.prefs.showFifth) {
            ...
            this._backgroundPitchRows[Math.round(Config.edo*Math.log2(3/2))].setAttribute("fill", this._doc.prefs.showFifth ? ColorConfig.fifthNote : ColorConfig.pitchBackground);
        }
        ...
        if (this._doc.prefs.showChannels) {
            if (!this._doc.song.getChannelIsMod(this._doc.channel)) {
                for (let channel: number = this._doc.song.pitchChannelCount + this._doc.song.noiseChannelCount - 1; channel >= 0; channel--) {
                    ...
                    const octaveOffset: number = this._doc.getBaseVisibleOctave(channel) * Config.edo;
