    ...
    private _exportToMidi(): void {
        ...
        for (const track of tracks) {
            ...
            if (isMeta) {
                ...
                while (numSharps > 6) numSharps -= Config.edo; // Range is (modulo 12) - 5. Midi supports -7 to +7, but I only have 12 options.
                                    ...
                                    for (let toneIndex: number = 0; toneIndex < toneCount; toneIndex++) {
                                        ...
                                        if (isDrumset) {
                                            ...
                                            if (preset != null && preset.midiSubharmonicOctaves != undefined) {
                                                nextPitch += Config.edo * preset.midiSubharmonicOctaves;
                                            } else if (isNoise) {
                                                nextPitch += Config.edo * (+EditorConfig.presetCategories.dictionary["Drum Presets"].presets.dictionary["taiko drum"].midiSubharmonicOctaves!);
                                            }
