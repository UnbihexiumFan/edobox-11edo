...
export class ImportPrompt implements Prompt {
	...
	private _parseMidiFile(buffer: ArrayBuffer): void {
		...
		while (key < 0) key += Config.edo; // Wrap around to a range from 0 to 11.
		key = key % Config.edo; // Wrap around to a range from 0 to 11.
		...
		for (let midiChannel: number = 0; midiChannel < 16; midiChannel++) {
			...
			if (isDrumsetChannel) {
				...
				const averagePitch: number = pitchSum / pitchCount;
					channel.octave = (isNoiseChannel || isModChannel) ? 0 : Math.max(0, Math.min(Config.pitchOctaves - 1, Math.floor((averagePitch / Config.edo))));
			}
