	...
	public static keyPosToPitch(doc: SongDocument, x: number, y: number, keyboardLayout: string): number | null {
		...
		switch (keyboardLayout) {
			...
			case "songScale":
				...
				pitchOffset = (y - 1 + Math.floor(x / scaleIndices.length)) * Config.edo + scaleIndices[(x + scaleIndices.length) % scaleIndices.length];
				break;
			case "pianoAtC":
		...
		const octaveOffset: number = Math.max(0, doc.song.channels[doc.channel].octave - 1) * Config.edo;
		...
		if (forcedKey != null) {
			...
			keyOffset = (forcedKey - keyBasePitch + 144) % Config.edo;
		}
